 ...
 * @author GNyabuto
 */
public class update_partner extends HttpServlet {
HttpSession session;
String partner_id,id_code,fco,lip_name,project_id,start_date,end_date,total_budget,currency,project_monitor,finance_monitor,is_active,project_name;
String message;
int code;
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
           ...
           String checker = "SELECT id FROM partner WHERE lip_name=? AND project_id=? && id!=?";
           ...
           if(conn.rs.next()){
             ...
             String adder = "UPDATE partner SET id_code=?,fco=?,lip_name=?,project_id=?,start_date=?,end_date=?,total_budget=?,currency=?,project_monitor=?,finance_monitor=? WHERE id=?";
             ...
             conn.pst.setString(11, partner_id);
    ...
    try {
        ...
        Logger.getLogger(update_partner.class.getName()).log(Level.SEVERE, null, ex);
    }
    }
