...
String id,area,description,is_active,timestamp;
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
           ...
           String get_areas = "SELECT id,name,description,is_active,timestamp FROM area ORDER BY name ";
           conn.pst = conn.conn.prepareStatement(get_areas);
           conn.rs = conn.pst.executeQuery();
           while(conn.rs.next()){
            id = conn.rs.getString("id");
            area = conn.rs.getString("name");
           ...
           finalobj.put("data", jarray);
            System.out.println("final array : "+finalobj);
